...
public class OnlineDAO {
	...
	public Vector getOnlineInfo_kr(String model_stylecode) {
		...
		try {	
			con = getConnection();
			sql = "select * from shoeinfo_onlineinfo where model_stylecode = ? AND country_name = ? order by online_method desc, online_start_date, online_end_date, online_start_time, online_end_time";
			pstmt = con.prepareStatement(sql);
	...
	public Vector getOnlineInfo_asia(String model_stylecode) {
		...
		try {	
			con = getConnection();
			sql = "select * from shoeinfo_onlineinfo where model_stylecode = ? AND country_region = ? AND NOT country_name = ? order by online_method desc, online_start_date, online_end_date, online_start_time, online_end_time";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, model_stylecode);
	...
	public Vector getOnlineInfo(String model_stylecode, String country_region) {
		...
		try {
			con = getConnection();
			sql = "select * from shoeinfo_onlineinfo where model_stylecode = ? AND country_region = ? order by online_method desc, online_start_date, online_end_date, online_start_time, online_end_time";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, model_stylecode);
